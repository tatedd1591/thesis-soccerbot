...
%         addpoints(banhright, xM(i) + 100*cos(thetaM(i)-pi/3), yM(i) + 100*sin(thetaM(i)-pi/3));
        ...
        xr=errorx*randn;
        yr=errory*randn;
        thetaM1(i)=thetaM(i)+theta0;
        if (thetaM1(i)>pi)
            thetaM1(i)=thetaM1(i)-2*pi;
        elseif (thetaM1(i)<-pi)
            thetaM1(i)=thetaM1(i)+2*pi;
        end
        ...
        e(2,i) = -(thetaM1(i)+thetar)+atan2(-(yM(i)+yr)+yt,-(xM(i)+xr)+xt);
        e(3,i) = -(thetaM1(i)+thetar)+(thetat);
        ...
        % Tinh van toc
        vf(i)=vR(i)*cos(e(2,i)+theta0);
        vn(i)=vR(i)*sin(e(2,i)+theta0);
        w(i);
        % Van toc 3 banh.
        vMi(i) = -vn(i)+R*w(i);
