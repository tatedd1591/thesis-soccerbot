...
void path(float xM,float yM,float thetaM,float x1, float y1);
...
//gia tri truyen
double vf=0,vn=0,w=0;   // Van toc tiep, phap, goc trong toa do robot
...
/*Định nghĩa macro SIZE_OF_ARRAY để lấy độ dài (số phần tử) trong mảng chỉ định*/
    ...
    // int array1[] = {5, 4, 7, 2, 8, 7, 3,32};
    // int array2[] = {1, 2, 3, 4, 5,43};
...
void calculate_error(void)
{
    ...
    //e1 =  (xR - xM)* cos(thetaM) + (yR - yM)*sin(thetaM);                                                                        
    //e2 = -(xR - xM)* sin(thetaM) + (yR - yM)*cos(thetaM);
    //e3 =  atan2(yt-y_start,xt-x_start) - thetaM;
    //v = vR*cos(e3)+k1*e1;
    //w = k2*-vR*e2/1000 + k3*sin(e3);
    pre_e2=e2;
	e2=pos;
    inte2=inte2+ (e2+pre_e2)*dt;
